...
<script>
    $( document ).ready(function() {
        ...
        $("#form").on("change", "select, input[type=radio]", function () {
            ...
            $.ajax({
                type: "POST",
                url: "/ax/assessment/assessmentquestioncheck",
        ...
        function fileUpload(event){
            ...
            $("#loading"+q_id).html(" Uploading...<img src='/../../v2assets/img/loading.gif' width='24px' height='24px' />");
            //get selected file
            ...
            for (var i = 0; i < files.length; i++) {
                ...
                var allowed_filetype = ["image/jpeg",
                    "image/png",
                    "image/x-png",
                ...
                var indexOf = jQuery.inArray( file.type, allowed_filetype );
                if(indexOf == (-1)){
                    //not found
                    $("#loading"+q_id).html("<span class=\'text-danger\'>you can not upload a file with this type.</span>");
                    return false;
                }
                if(file.size > 11762150 ){ //11762150
                    //check file size (in bytes)
                    $("#loading"+q_id).html("<span class='text-danger'>Sorry, your file is too large (>11 MB)</span>");
                }else{
                    ...
                    xhr.open('POST', "<?php echo DOMAIN_PATH ;?>/ax/assessment/dropzoneupload", true);
                    xhr.send(data);
                    xhr.onload = function () {
                        ...
                        if(xhr.status === 200){
                            ...
                            if(response.status == 'success'){
                                $("#loading"+q_id).html('');
                                $("#status"+q_id).append(response.filename+"File uploaded"+"<br/>");
                                //response.status
                                // $( "#result" ).load( "list.php" );
                            }else if(response.status == 'file_err'){
                                $("#status"+q_id).append("<span class='text-danger'>You cannot upload this file type to this workspace.</span>");
                            }else{
                                $("#status"+q_id).append("<span class='text-danger'>A problem occured, please try again.</span>");
                            }
                        }
                    };
                }
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $response_code = $_GET['existing_response'];
...
<div class="portlet light ">
    <div class="portlet-title">
        <div class="caption">
            <?php if ($existing_response) { ?>
                ...
                <a href="<? echo DOMAIN_PATH;?>/controller/assessment/engage?aoid=<?php echo $object_id; ?>&existing_response=<?php echo $existing_response; ?>">[Edit Response]</a>
            <?php } ?>
            ...
            if (pb_assessment_has_sections($assessment_id)) { ?>
                ...
                <select name="section" id="section" class="form-control">
                    <option value=""></option>
                    <?php
                    //$statement = $db->prepare("select * from assessment_section where parent_assessment_id = :aoid and item_status<>'deleted'");
                    $statement = $db->prepare("select object_id,title from assessment_section where parent_assessment_id=:aoid and section_status <> 'deleted' order by order_id ASC");
    ...
    <div class="portlet-body form">
        ...
        <form id="form" name="form" class="form-horizontal" role="form" action="<?php echo DOMAIN_PATH ;?>/controller/assessment/engage" method="POST" enctype="multipart/form-data">
            <?php
            ...
            ?>
                ...
                <div class="form-group"><label class="control-label col-md-1">Friendly Name</label>
                    <div class="col-md-11"><h4><input name="friendly_name" type="text" size="80" value="" style="width:95%" class="form-control"></h4>
                    </div>
                </div>
            <?php } ?>
            <input type="hidden" id="unique_access_id" name="unique_access_id" value="<?php echo $unique_access_id; ?>">
            <input type="hidden" id="response_code" name="response_code" value="<?php echo $response_code; ?>">
            ...
            if (pb_assessment_has_sections($aoid)) {
                ...
                $section_numeral = "1";
                $statement = $db->prepare("select object_id,title from assessment_section where identikey=:s_identikey and parent_assessment_id=:aoid and section_status <> 'deleted' order by order_id ASC");
                $statement->bindParam(':aoid', $aoid);
            ...
            if ($existing_response) {
                ?>
                <input type="hidden" id="existing_response" name="existing_response" value="<?php echo $existing_response; ?>">
                <?php
            }
                    ...
                    <div class="pl-auto pr-auto">
                        <div class="row align-items-center">
                            <div class="col-md-12">
                                ...
                                <button type="submit" class="btn green" value="save_exit" name="submit_button"> Draft </button>
